...
export function SnakeGame({ onClose }: SnakeGameProps) {
  ...
  useEffect(() => {
    ...
    const handleKeyPress = (e: KeyboardEvent) => {
      ...
      switch (e.key) {
        case 'ArrowUp':
          if (lastQueuedDirection.y === 0) newDirection = { x: 0, y: -1 }
          break
        case 'ArrowDown':
          if (lastQueuedDirection.y === 0) newDirection = { x: 0, y: 1 }
          break
        case 'ArrowLeft':
          if (lastQueuedDirection.x === 0) newDirection = { x: -1, y: 0 }
          break
        case 'ArrowRight':
          if (lastQueuedDirection.x === 0) newDirection = { x: 1, y: 0 }
          break
      ...
      // Add to queue if valid and not already in queue
      if (newDirection) {
        // Check if this direction is already the last in queue
        const lastInQueue = directionQueue.current[directionQueue.current.length - 1]
        if (!lastInQueue || lastInQueue.x !== newDirection.x || lastInQueue.y !== newDirection.y) {
  ...
  return (
    <div className="p-2 bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 flex flex-col items-center justify-center w-full h-full">
      ...
      <div className="relative bg-gradient-to-br from-gray-800 to-gray-900 border-2 border-gray-600 rounded-xl shadow-2xl overflow-hidden flex-shrink-0 mx-auto" style={{ width: '320px', height: '320px' }}>
        <div 
          className="grid gap-0.5 p-0.5 w-full h-full" 
          style={{ 
          ...
          {Array.from({ length: GRID_SIZE * GRID_SIZE }).map((_, i) => {
            ...
            return (
              <motion.div
                ...
                initial={false}
                animate={{
                  scale: isFood && foodEaten ? [1, 1.3, 1] : isHead ? [1, 1.05, 1] : 1,
                }}
